...
<body class="" style="background-color: #3a3a3a ;">
    ...
    <nav class="navbar navbar-ligh bg-dark text-white" style="">
    ...
    <script>
            ...
            fetch('getsolution.php') // PHP 파일 경로로 변경
                ...
                .then(data => {
                    ...
                    data.forEach(item => {
                        ...
                        s_box.className = "col-2 mb-2 mx-1 pb-3 plist border border-3";
                        // 최소/최대 너비 설정으로 박스 크기 제어
                        s_box.style.minWidth = "220px";
                        s_box.style.maxWidth = "245px";
                        s_box.style.borderRadius = "8px";
                        s_box.style.boxShadow = "0 4px 6px rgba(0, 0, 0, 0.1)";
                        s_box.style.backgroundColor = "#fff";
                        s_box.style.transition = "transform 0.2s ease-in-out";

                        // 호버 효과 추가
                        s_box.addEventListener('mouseenter', () => {
                            s_box.style.transform = "translateY(-5px)";
                        });
                        s_box.addEventListener('mouseleave', () => {
                            s_box.style.transform = "translateY(0)";
                        });


                        var s_title_box = document.createElement('div');
                        s_title_box.className = "d-flex justify-content-center p-2";
                        s_title_box.style.cursor = "pointer";
                        s_title_box.style.background = "linear-gradient(135deg, #ff4b2b, #ff416c)";
                        s_title_box.style.borderRadius = "6px 6px 0 0";
                        s_title_box.style.transition = "opacity 0.2s ease";
                        ...
                        // 호버 효과 추가
                        s_title_box.addEventListener('mouseenter', () => {
                            s_title_box.style.opacity = "0.9";
                        });
                        s_title_box.addEventListener('mouseleave', () => {
                            s_title_box.style.opacity = "1";
                        });

                        var s_title = document.createElement('span');
                        s_title.textContent = item.s_title;
                        s_title.className = "fw-bold fs-5 text-white";

                        var s_content_box = document.createElement('div');
                        s_content_box.className = "p-3";

                        var s_label = document.createElement('label');
                        s_label.className = "form-label d-flex justify-content-between mb-2";
                        s_label.setAttribute("for", "sol" + item.s_id);
                        s_label.innerHTML = `
                            <span class="badge bg-secondary">난이도: ${item.s_difficulty}</span>
                            <span class="badge bg-info">점수: ${item.s_point}</span>
                        `;

                        var s_input = document.createElement('input');
                        s_input.className = "form-control ptext mt-2";
                        s_input.id = "sol" + item.s_id;
                        s_input.style.transition = "all 0.3s ease";

                        if (item.s_clear == 'true') {

                            s_title_box.style.background = "linear-gradient(135deg, #32CD32, #228B22)";
                            s_input.value = 'CLEAR!';
                            s_input.type = 'text';
        ...
        document.getElementById('list-userdata-list').addEventListener('click', () => {
            ...
            fetch('getUCdata.php') // PHP 파일 경로로 변경
                .then(response => response.json())
                .then(data => {
                    console.log(data); // 받아온 데이터 출력
                    // 추가적인 데이터 처리...
